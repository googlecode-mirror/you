...
<table cellspacing="0" cellpadding="0" align="center" class="browse"><tr><th style='width:18px'><?php if ($pos = strrpos(substr($_GET['thedir'], 0, -1), "/"))$folder_up='&amp;thedir='.substr($_GET['thedir'], 0, $pos).'/';?><a href="<?php echo $_SERVER['PHP_SELF'].'?type='.THE_TYPE.'&amp;sa='.STAND_ALONE.$folder_up;?>" title="<?php echo TXT_BACK;?>"><img src="img/folder_up.gif" width="16" height="16" alt="<?php echo TXT_BACK;?>" /></a></th><th><?php echo TXT_NAME?></th><th><?php echo TXT_SIZE?></th><th><?php echo TXT_TYPE?></th><th><?php echo TXT_DATE?></th>
...
for($ii=0;$ii<count($dir_name);$ii++)
		...
		echo '<tr class="'.$row_color.'">
			...
			<td colspan="3" valign="top"><a href="'.$_SERVER['PHP_SELF'].'?type='.THE_TYPE.'&amp;thedir='.$dir_name_path[$ii].'/&amp;sa='.STAND_ALONE.'" title="'.TXT_BROWSE.'">'.$dir_name[$ii].'</a></td>
...
<?php
	(!STAND_ALONE) ? $onsubmit = 'insert_theurl()' : $onsubmit = "get_theurl('".CALLBACK_FUNCTION."')" ;
?>
<form action="" name="previewform" id="previewform" onsubmit="javascript:<?php echo $onsubmit?>;return false;">
	<input type='hidden' name='selected_url' id='selected_url' />
	<input type="submit" name="go" id="go" value="<?php echo TXT_INSERT?>" class="insert_link" />
</form>
<span id='do_download' name='do_download'></span>
