...
function clean_filename($filename){
    ...
    $filename = str_replace("'", '_', $filename); // Replace ' by _
...
while (($dirname = readdir($dh)) !== false)
{
	// Added by Partikule
	$show_dir = true;
	if ($tinybrowser['show_ionize_thumb_dir'] == true )
	{
		$show_dir = ($dirname != $tinybrowser['thumbdir'] ) ? true : false;
	}
	else
	{
		$show_dir = (substr($dirname,0,5) != $tinybrowser['thumbdir']) ? true : false;
	}
	// End Added by Partikule

	if($dirname != '.' && $dirname != '..' && is_dir($root.$tree.$branch.$dirname) && $show_dir == true)
	{
		$filenum=0;
		foreach($filetypes as $filetype)
		   {
      ...
      if(strlen($indent)>0) $indent .= '&raquo; ';
