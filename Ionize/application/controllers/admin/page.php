...
class Page extends MY_admin
{
	 ...
	 * @var array
	 */
	protected $no_htmlspecialchars = array();
	...
	{	
		...
		$this->template['menus'] =	form_dropdown('id_menu', $datas, false, 'id="id_menu" class="select"');

		
		// Dropdown parents
//		$datas = $this->page_model->get_lang_list(false, Settings::get_lang('default'));
...
//			$this->template['parent_array'] = $this->page_model->get_parent_array($id);
			...
			$parents = array('0' => '/');
			($parents_array = $this->structure->get_parent_select($datas, $id) ) ? $parents += $parents_array : '';
			$this->template['parents'] = form_dropdown('id_parent', $parents, $this->template['id_parent'], 'class="select" id="id_parent"');
			...
			*/
			 ...
			 * Linked articles
			 */
			$this->template['articles'] = $this->article_model->get_lang_list(array('id_page'=>$id), Settings::get_lang('default'));
		...
		if ($this->_check_before_save() == TRUE)
		{
			...
			else
			{
				...
				if ( empty($id))
				{
					$this->update[] = array(
						'element' => 'mainPanel',
						'url' => site_url('admin/page/edit/'.$this->id),
	...
	function get_parents_select($id_menu, $id_parent=0)
	{
		...
		$parents = array('0' => '/');
		($parents_array = $this->structure->get_parent_select($datas, 0) ) ? $parents += $parents_array : '';
