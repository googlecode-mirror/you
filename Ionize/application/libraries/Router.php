...
class CI_Router
{
	...
	public function _set_routing()
	{
		...
		else
		{
			...
			if($this->module_path)
			{
				...
				Finder::$paths = array_merge(array($this->config->item('module_path') . $this->module_path . '/'), Finder::$paths);
			}
	...
	public function match_to_routes($segments)
	{
		...
		foreach($files as $f)
		{
			...
			// merge the routes
//			$this->routes = ( ! isset($route) OR ! is_array($route)) ? $this->routes : array_merge($route, $this->routes);
	...
	public function set_default_language()
	{
		...
		if ($segments[0] == 'admin' OR (isset($segments[1]) && $segments[1] == 'admin') )
		{
			$this->lang_key = $this->config->item('default_lang');
		}
		else
...
//		return $this->class . '_Controller';
	...
	/**
	 * Replace the original CodeIgniter.php core hack
	 *
	 * @author : BrianDHall
	 *
	 */
