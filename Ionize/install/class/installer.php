...
class Installer
{
	...
	function check_config()
	{
		...
		// MCrypt
		$this->template['mcrypt'] = ( ! function_exists('mcrypt_encrypt')) ? false : true;
		...
		$this->template['write_config_medias'] = is_really_writable(ROOTPATH . 'application/config/medias.php');
...
//				$this->template['next'] = false;
	...
	function migrate()
	{
		...
		if ( ! isset($_POST['action']))
		{
			...
			{
				if (in_array('migration_0.9.4_0.9.5.xml', $migration_files)) $this->template['database_migration_from'] = lang('database_migration_from_094');			
				if (in_array('migration_0.93_0.9.4.xml', $migration_files)) $this->template['database_migration_from'] = lang('database_migration_from_093');			
				if (in_array('migration_0.92_0.93.xml', $migration_files)) $this->template['database_migration_from'] = lang('database_migration_from_092');			
				if (in_array('migration_0.90_0.92.xml', $migration_files)) $this->template['database_migration_from'] = lang('database_migration_from_090');
	...
	function _get_migration_files()
	{
		...
		// The database don't contains correct tables -> error !
